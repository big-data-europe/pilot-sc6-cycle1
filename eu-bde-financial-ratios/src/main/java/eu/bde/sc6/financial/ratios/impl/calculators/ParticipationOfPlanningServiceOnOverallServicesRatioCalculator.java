 ...
 */
public class ParticipationOfPlanningServiceOnOverallServicesRatioCalculator extends ArithmeticFinancialRatioCalculatorBase {
    String QUERY_ARITHM = ""
            + "SELECT * "
            + "FROM <<dataGraph>>  "
            + "WHERE {"
            ...
            + "         ) "
            + "         FILTER(STRSTARTS(STR(?kaeuri), \"http://linkedeconomy.org/resource/KAE/<year>/Income/4/40\"))"            
            + "     }"
            + " }"
            + " {"
    ...
    public ParticipationOfPlanningServiceOnOverallServicesRatioCalculator(URI financialRatio, SPARQLRepository dataRepository, SPARQLRepository pptRepository) {
