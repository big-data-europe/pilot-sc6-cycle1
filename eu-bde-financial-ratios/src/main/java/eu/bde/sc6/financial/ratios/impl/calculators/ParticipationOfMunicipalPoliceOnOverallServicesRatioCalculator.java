 ...
 */
public class ParticipationOfMunicipalPoliceOnOverallServicesRatioCalculator extends ArithmeticFinancialRatioCalculatorBase {
    String QUERY_ARITHM = ""
            + "SELECT * "
            + "FROM <<dataGraph>>  "
            + "WHERE {"
            ...
            + "         ) "
            + "         FILTER(STRSTARTS(STR(?kaeuri), \"http://linkedeconomy.org/resource/KAE/<year>/Income/5/50\"))"            
            + "     }"
            + " }"
            + " {"
    ...
    public ParticipationOfMunicipalPoliceOnOverallServicesRatioCalculator(URI financialRatio, SPARQLRepository dataRepository, SPARQLRepository pptRepository) {
