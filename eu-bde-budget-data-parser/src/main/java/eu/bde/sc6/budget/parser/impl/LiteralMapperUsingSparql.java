...
/**
 * A mapper that, given a list of RDF statements and a PP thesaurus, returns a list
 * of RDF statements where the literals have been substituted, when possible, for
 * URIs of concepts in the thesaurus.
 * 
 *  The input is scanned for statements of the form ?s P literal where P is in 
 * the objects set labelsToSubstitute (given in the constructor).
 * If the literal is found in the PP thesaurus in a statement of the form 
 *   ?S2 P2 literal   where P2 is from the objects matchingPredicates list, then
 * the original statement is substituted in the output for the statement ?s P ?S2
 * Otherwise, the original statement is left untouched.
...
public class LiteralMapperUsingSparql {
    ...
    */
    ...
    private Set<String> getAllLiterals(List<Statement> data)
    {
